...
The MCP endpoint is served at `POST /mcp`; the A2A endpoint (agent card + JSON-RPC) is served at `GET /.well-known/agent-card.json` / `POST /a2a`; `GET /health` reports DB connectivity. `GET /api/meal-history` (`?from=YYYY-MM-DD&to=YYYY-MM-DD`, half-open, Asia/Tokyo calendar dates) and `GET /api/nutrient-definitions` serve read-only JSON for the web frontend. `GET /api/profile` serves the same, and `PATCH /api/profile` patches it (omitted fields keep their current value; `dailyTargets: null` clears it).
