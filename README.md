...
The MCP endpoint is served at `POST /mcp`; the A2A endpoint (agent card + JSON-RPC) is served at `GET /.well-known/agent-card.json` / `POST /a2a`; `GET /health` reports DB connectivity.
