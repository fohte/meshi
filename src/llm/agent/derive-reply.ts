...
// Reads the turn's final reply back out of the raw message list produced by
// createMeshiDomainAgent's ReAct loop. There is no mandatory
// structured-output tool call any more: the last AI message in the turn
// carries the model's free-form reply directly in its text, and optionally a
// call to request_user_input signaling that the reply is a question the
// user must answer before the agent can proceed (see system-prompt.ts and
// request-user-input-tool.ts). Returns null when the turn produced no
// usable text at all — callers treat this as a hard failure (see
// a2a/agent-executor.ts and domain-agent-orchestrator.ts).
export const deriveAgentReply = (
  messages: ReadonlyArray<AgentInvokeMessage> | undefined,
  onThinkBlockLeaked: () => void,
): AgentReply | null => {
  const turnMessages = findTurnMessages(messages)
  const lastAiMessage = turnMessages.findLast((m) => m.getType() === 'ai')
  if (lastAiMessage === undefined) return null

  const { text, stripped } = stripThinkBlocks(lastAiMessage.text)
  if (stripped) onThinkBlockLeaked()
  if (text === '') return null

  const requestedInput = (lastAiMessage.tool_calls ?? []).some(
    (call) => call.name === REQUEST_USER_INPUT_TOOL_NAME,
  )
  return { status: requestedInput ? 'input_required' : 'completed', text }
}
